...
const AddTrainingAndUpdate = ({
  ...
  const selectedCategory = categoryOptions?.find(
    (item) => item.name === initialData?.categoryName
  );
  ...
  const validationSchema = Yup.object({
    title: Yup.string()
      .required("Kursun adı vacibdir")
      .test(
        "not-only-numbers",
        "Yalnız rəqəmlərdən ibarət ad qəbul olunmur",
        (value) => !/^\d+$/.test(value)
      )
      .matches(
        /^[\p{L}\s\d\p{P}]+$/u,
        "Yalnız hərflər, rəqəmlər və işarələr qəbul olunur"
      ),
  ...
  const handleSubmit = async (values: typeof initialValues) => {
    try {
      ...
      Swal.fire(
        "Xəta",
        isEdit ? "Sitat yenilənə bilmədi" : "Sitat əlavə edilə bilmədi",
        "error"
      );
    }
  };
